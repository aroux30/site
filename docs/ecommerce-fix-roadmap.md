...
| BE-20 | P2 | INTEGRATION GAP | Returns/RMA | RMA lifecycle has no admin API (approve/receive/inspect/refund unreachable via HTTP) | Only RMA creation exposed; transitions are service-level only | `orders/api/routes.py`, `returns_service.py` | none | Admin RMA endpoints + RBAC + restock on return completion (merges P3-01) | Unit + integration | Medium | OPEN (discovered 2026-09-11) |

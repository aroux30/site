...
|---|---|:---:|:---:|---|
| **Inventory Concurrency** | Zero overselling with `stock=1` | Level 5 | **LEVEL 6** | `test_real_postgres_100_concurrent_inventory_reservations` executed on live DB: 1 success, 99 rejected |
| **Coupon Concurrency** | Zero double-redemption of 1-use code | Level 5 | **LEVEL 6** | `test_real_postgres_50_concurrent_single_use_coupon_redemptions` on live DB: 1 success, 49 rejected |
...
| **Payment Security** | Mock provider strictly disabled in prod | Level 3 | **LEVEL 6** | `test_mock_payment_provider_strictly_fails_closed_in_production` raises `ValueError` under `ENVIRONMENT=production` |
| **Database Migration** | Deterministic 74 tables from scratch | Level 3 | **LEVEL 6** | `CREATE DATABASE test_fresh_migration; alembic upgrade head` executed on live PostgreSQL engine |

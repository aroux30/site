...
MANDATORY_INDEXES: list[IndexSpec] = [
    ...
    IndexSpec(
        "orders", "order_number", "ix_orders_order_number", "Order lookup by unique reference"
    ),
    ...
    IndexSpec(
        ...
        "Duplicate order submission defense",
    ),
    IndexSpec("order_items", "order_id", "ix_order_items_order_id", "Eager loading order items"),
    IndexSpec(
        "order_items", "variant_id", "ix_order_items_variant_id", "Order line item stock lookup"
    ),
    IndexSpec("audit_logs", "actor_id", "ix_audit_logs_actor_id", "Admin audit search by user"),
    IndexSpec(
        "audit_logs", "action", "ix_audit_logs_action", "Audit filtering by security action"
    ),
    IndexSpec(
        "audit_logs",
        "created_at",
    ...
    IndexSpec(
        "shipping_methods", "slug", "ix_shipping_methods_slug", "Carrier method resolution by slug"
    ),
]
...
def test_mandatory_indexes_coverage():
    ...
    for spec in MANDATORY_INDEXES:
        ...
        assert spec.index_name in declared, (
            f"Missing critical performance index: {spec.table}.{spec.column} -> {spec.index_name}"
        )

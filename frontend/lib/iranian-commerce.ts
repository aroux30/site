...
export function evaluateCoupon(code: string, currentTotal: number): CouponResult {
  const normalized = code.trim().toUpperCase();

  if (normalized === "WELCOME10") {
    const discount = Math.min(Math.round(currentTotal * 0.1), 500000);
    return {
      valid: true,
      discountAmount: discount,
      discountPercent: 10,
      message: `کد تخفیف ۱۰٪ با موفقیت اعمال شد (سقف ۵۰۰,۰۰۰ تومان)`,
    };
  }

  if (normalized === "NOWRUZ") {
    if (currentTotal < 2000000) {
      return {
        valid: false,
        discountAmount: 0,
        message: "حداقل مبلغ سفارش برای این کد ۲,۰۰۰,۰۰۰ تومان است",
      };
    }
    return {
      valid: true,
      discountAmount: 300000,
      message: "کد تخفیف ۳۰۰,۰۰۰ تومانی با موفقیت اعمال شد",
    };
  }
  ...
  if (normalized === "FREESHIP") {
    return {
      ...
      message: "هزینه ارسال برای شما رایگان شد",
    };
  }
